...
export const useAlarmStore = defineStore(
  ...
  () => {
    ...
    // 안 읽은 알림 목록 불러오기
    const getUnReadAlarmList = async () => {
      try {
        const res = await axios.get(`http://localhost:8080/notify/list`, { params: { loginId }});
        const sortedAlarmList = res.data.data.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        alarmUnReadList.value = sortedAlarmList;
        console.log("sortedAlarmList.length : " + sortedAlarmList.length);
        console.log("alarmUnReadList : ", alarmUnReadList.value);
        console.log("getUnReadAlarmList 호출 종료 ..");
      } catch (error) {
